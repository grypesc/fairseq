...
class RQL(BaseFairseqModel):
    ...
    def build_model(cls, args, task):
        ...
        model = RQL(net, device, TESTING_EPISODE_MAX_TIME, len(target_vocab), discount,
                    torch.tensor([target_vocab.eos_index]).to(device),
                    torch.tensor([target_vocab.bos_index]).to(device),
                    torch.tensor([target_vocab.pad_index]).to(device),
                    torch.tensor([source_vocab.eos_index]).to(device),
                    torch.tensor([source_vocab.bos_index]).to(device),
                    torch.tensor([source_vocab.pad_index]).to(device)).to(device)
